...
c1_1_encode_10_test() ->
    ...
    Encoded = eh2_hpack:encode_integer(10, 5),
    ?assertEqual(<<10>>, Encoded).

c1_1_decode_10_test() ->
    %% Decode with 5-bit prefix
    {ok, Value, <<>>} = eh2_hpack:decode_integer(<<10>>, 5),
...
c1_3_encode_42_test() ->
    ...
    Encoded = eh2_hpack:encode_integer(42, 8),
    ?assertEqual(<<42>>, Encoded).

c1_3_decode_42_test() ->
    {ok, Value, <<>>} = eh2_hpack:decode_integer(<<42>>, 8),
...
integer_roundtrip_test_() ->
    ...
    [?_assertEqual({ok, V, <<>>}, eh2_hpack:decode_integer(eh2_hpack:encode_integer(V, P), P))
     || {V, P} <- TestCases].
...
c2_1_literal_indexed_test() ->
    ...
    {Encoded, Ctx1} = eh2_hpack:encode(Headers, Ctx),
    ...
    {Encoded2, _Ctx3} = eh2_hpack:encode(Headers, Ctx1),
...
c2_2_literal_no_index_decode_test() ->
    ...
    Ctx = eh2_hpack:new_context(),
    {ok, Headers, _Ctx1} = eh2_hpack:decode(Binary, Ctx),
...
c2_3_literal_never_indexed_decode_test() ->
    ...
    Ctx = eh2_hpack:new_context(),
    {ok, Headers, Ctx1} = eh2_hpack:decode(Binary, Ctx),
    ...
    {Encoded, _Ctx2} = eh2_hpack:encode(Headers, Ctx1),
...
c3_1_first_request_test() ->
    ...
    %% Encode
    {Encoded1, Ctx1} = eh2_hpack:encode(Headers1, Ctx0),

    %% Decode and verify
    {ok, Decoded1, DecCtx1} = eh2_hpack:decode(Encoded1, eh2_hpack:new_context()),
    ...
    {Encoded2, Ctx2} = eh2_hpack:encode(Headers2, Ctx1),
    {ok, Decoded2, DecCtx2} = eh2_hpack:decode(Encoded2, DecCtx1),
    ...
    {Encoded3, _Ctx3} = eh2_hpack:encode(Headers3, Ctx2),
    {ok, Decoded3, _DecCtx3} = eh2_hpack:decode(Encoded3, DecCtx2),
...
c4_huffman_encoding_test() ->
    ...
    HuffmanEncoded = eh2_hpack:huffman_encode(TestString),

    %% Huffman should be smaller or equal
    ?assert(byte_size(HuffmanEncoded) =< byte_size(TestString)),

    %% Round-trip test
    {ok, Decoded} = eh2_hpack:huffman_decode(HuffmanEncoded),
...
c4_huffman_common_strings_test_() ->
    ...
    [?_assertEqual({ok, S}, eh2_hpack:huffman_decode(eh2_hpack:huffman_encode(S)))
     || S <- Strings].
...
c5_response_sequence_test() ->
    ...
    {Encoded1, Ctx1} = eh2_hpack:encode(Headers1, Ctx0),
    {ok, Decoded1, DecCtx1} = eh2_hpack:decode(Encoded1, DecCtx0),
    ?assertEqual(Headers1, Decoded1),
    ...
    {_Encoded3, _Ctx3} = eh2_hpack:encode(Headers3, Ctx2),
    {ok, Decoded3, _DecCtx3} = eh2_hpack:decode(_Encoded3, DecCtx2),
    ?assertEqual(Headers3, Decoded3).
...
c6_huffman_response_test() ->
    ...
    Ctx = eh2_hpack:new_context(),
    {Encoded, _Ctx1} = eh2_hpack:encode(TestHeaders, Ctx),
...
dynamic_table_size_update_test() ->
    ...
    Ctx = eh2_hpack:new_context(8192),
    ?assertEqual(8192, eh2_hpack:get_max_table_size(Ctx)),

    %% Reduce table size
    Ctx1 = eh2_hpack:set_max_table_size(256, Ctx),
    ?assertEqual(256, eh2_hpack:get_max_table_size(Ctx1)).

dynamic_table_eviction_test() ->
    %% Use small table to force eviction
    Ctx0 = eh2_hpack:new_context(128),

    %% Add headers that will exceed table size
    Headers1 = [{<<"a-long-header-name">>, <<"a-long-header-value">>}],
    {_Encoded1, Ctx1} = eh2_hpack:encode(Headers1, Ctx0),

    Headers2 = [{<<"another-long-header">>, <<"another-long-value">>}],
    {_Encoded2, Ctx2} = eh2_hpack:encode(Headers2, Ctx1),

    %% Encoding the first header again should be a full literal
    %% (not indexed) because it was evicted
    {Encoded3, _Ctx3} = eh2_hpack:encode(Headers1, Ctx2),
...
static_table_entries_test_() ->
    ...
    [begin
        ...
        Ctx = eh2_hpack:new_context(),
        {ok, [{Name, Value}], _} = eh2_hpack:decode(Binary, Ctx),
...
empty_headers_test() ->
    Ctx = eh2_hpack:new_context(),
    {Encoded, _Ctx1} = eh2_hpack:encode([], Ctx),
    ?assertEqual(<<>>, Encoded),

    {ok, Decoded, _Ctx2} = eh2_hpack:decode(<<>>, Ctx),
    ?assertEqual([], Decoded).
...
invalid_index_test() ->
    ...
    Ctx = eh2_hpack:new_context(),
    Result = eh2_hpack:decode(Binary, Ctx),
